...
module.exports = function preset(context, opts = {}) {
  ...
  return {
    plugins: [
      /* es2015 */
      modules === 'commonjs' && [require('@gerhobbelt/babel-plugin-transform-modules-commonjs'), optsLoose],

      edge && require('@gerhobbelt/babel-plugin-transform-arrow-functions'), // needed for function-name
      edge && require('@gerhobbelt/babel-plugin-transform-function-name'),

      /* es2018 */
      es2018 &&
        (edge
          ? [require('@gerhobbelt/babel-plugin-proposal-object-rest-spread'), { loose, useBuiltIns: true }]
          : require('@gerhobbelt/babel-plugin-syntax-object-rest-spread')),

      es2018 && [
        require('@gerhobbelt/babel-plugin-proposal-unicode-property-regex'),
        { useUnicodeFlag: true },
      ],

      es2018 && edge
        ? require('@gerhobbelt/babel-plugin-proposal-async-generator-functions')
        : require('@gerhobbelt/babel-plugin-syntax-async-generators'),

      /* shippedProposals */
      shippedProposals &&
        shippedProposals &&
        require('@gerhobbelt/babel-plugin-syntax-optional-catch-binding'),
    ].filter(Boolean),
  };
};
